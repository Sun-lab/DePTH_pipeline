...
---
title: "Extract TCRs based on cancer reactive gene signature information for CD4+ T cells, keep v gene part"
author: "Wei Sun"
date: "`r Sys.Date()`"
output: 
  html_document:
...
```{r}
cd4T_files = list.files(paste0(data_dir, "processed/"), 
                        pattern="10X.CD4.txt.gz")
cd4T_files
...
cluster_p[["ESCA"]] = c(7)
cluster_n[["ESCA"]] = c(1,2,5)

cluster_p[["MM"]] = c(8)
cluster_n[["MM"]] = c(1,6)

cluster_p[["PACA"]] = c(5)
cluster_n[["PACA"]] = c(2,3,4)

cluster_p[["RC"]] = c(7)
cluster_n[["RC"]] = c(3,5)

cluster_p[["THCA"]] = c(6,8)
cluster_n[["THCA"]] = c(1,4)

cluster_p[["UCEC"]] = c(8)
cluster_n[["UCEC"]] = c(3,4,5,6)
...
for(f1 in cd4T_files){
  cancer = str_extract(f1, "(?<=_)\\w+(?=_10X.CD4.txt.gz)")
...
fwrite(pos_tcr, file=file.path(data_dir, "TCRs_CD4_pos_with_v_gene_weighted.csv"))
fwrite(neg_tcr, file=file.path(data_dir, "TCRs_CD4_neg_with_v_gene_weighted.csv"))
```
