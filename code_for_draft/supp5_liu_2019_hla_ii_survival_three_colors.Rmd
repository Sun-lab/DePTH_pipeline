---
title: "Liu et al. 2019 data hla ii plots with survival, three colors"
author: "Si Liu"
date: "`r Sys.Date()`"
output: 
  html_document:
...
```{r}
df_kept = read.csv("../results/st24_liu_2019_hla_ii_depth_aa_set_breadth.csv", header = TRUE)
dim(df_kept)
colnames(df_kept)
...
df = cbind(df, df_kept[, c(colnames(df_kept))[c(48, 51:54)]])
dim(df)
head(df)
...
for (k in 1:length(data_list)){
  ...
  fig_filename = paste0("../figures/depth_draft/supp5_HLA_II_log_nonsyn_mut_survival_scatter_", data_names[k], ".pdf")
